...
public class AddDealActivity extends AppCompatActivity {
	...
	private String imgPath, fileName = "not_available", business_id = "",
			is_display_in_city = "", deal_catid = "";
	private Bitmap bitmap;
	private static int RESULT_LOAD_IMG = 1;
	private Intent intent;
	private boolean is_deal_added = false;

	private Toolbar toolBar;
	private TextInputLayout input_layout_deal_title, input_layout_deal_url,
			input_layout_dealdesc, input_layout_deal_mainprice,
			input_layout_deal_price;
	private EditText input_deal_title, input_deal_desc, input_deal_url,
			input_deal_mainprice, input_deal_price;
	private Button selectimage, selectdealcat, selectenddate, btncancel,
			btnsubmit;
	private boolean is_image_selected = false, is_deal_cat_selected = false,
			is_select_enddate_selected = false;
	...
	private static String url_add_deal = "http://www.pindout.com/mobi/pindout/add_deal.php";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		edit.putString(strloginprefs, "1");
		edit.commit();

		input_layout_deal_title = (TextInputLayout) findViewById(R.id.input_layout_dealtitle);
		input_layout_deal_url = (TextInputLayout) findViewById(R.id.input_layout_dealurl);
	...
	public void SubmitBtn(View v) {
		...
		if (is_deal_cat_selected == false) {
			Toast.makeText(getApplicationContext(), "Select Deal Category",
					Toast.LENGTH_SHORT).show();
			return;
		}

		if (is_select_enddate_selected == false) {
			Toast.makeText(getApplicationContext(), "Select Deal End Date",
					Toast.LENGTH_SHORT).show();
			return;
		}
		CityDealsDialog();
	...
	private void showDate(int year, int month, int day) {
		...
		is_select_enddate_selected = true;
	}
	...
	public void makeHTTPCall() {
		...
		endpDialog = new ProgressDialog(AddDealActivity.this);
	...
	class AddDeal extends AsyncTask<String, String, String> {
		private ProgressDialog pDialog;
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(AddDealActivity.this);
			pDialog.setMessage("Adding deal...");
			pDialog.setIndeterminate(false);
			pDialog.setCancelable(false);
			pDialog.show();
		}
		...
		protected String doInBackground(String... args) {
			...
			params.add(new BasicNameValuePair("deal_desc", input_deal_desc
					.getText().toString()));
			params.add(new BasicNameValuePair("deal_image", business_id + "_"
					+ fileName));
			...
			JSONObject json = jParser.makeHttpRequest(url_add_deal, "POST",
					params);
	...
	protected void onDestroy() {
		// TODO Auto-generated method stub
		super.onDestroy();
		if (endpDialog != null && is_deal_added) {
			endpDialog.dismiss();
			Toast.makeText(
					getApplicationContext(),
					input_deal_title.getText().toString()
							+ " deal added successfully", Toast.LENGTH_SHORT)
					.show();
			input_deal_mainprice.setText("");
			input_deal_price.setText("");
			input_deal_title.setText("");
			input_deal_url.setText("");
			selectdealcat.setText("");
